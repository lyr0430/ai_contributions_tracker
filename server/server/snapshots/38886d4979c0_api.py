...
async def save_analysis_endpoint(req: AnalysisRequest):
    ...
    for f in req.files:
        ...
        record = OperationRecord(
            project_name=req.project_name,
            project_path=req.project_path,
            session_id=f"push_{req.commit_hash[:8]}",
            operation_id=op_id,
            timestamp=req.commit_time,
            operation="push_file",
            file_path=f["path"],
            diff_added="[]",
            diff_removed="[]",
...
async def get_report(
    ...
    with Session(engine) as session:
        ...
        if group_by == "commit":
            ...
            total = sum(c["ai_pure_lines"] + c["ai_modified_lines"] + c["mixed_lines"] + c["human_lines"] for c in commits.values())
